with basic as(
	select
		...
		case when incexp_header.type_id = 2
			then incexp_position.amount_absolute
			else -1 * incexp_position.amount_absolute
		end as amount
...
select
	...
	,sum(coalesce(basic_grouped.amount, 0)) over (order by year_months.year_month) as "Bilans kumulacyjny [PLN]"
from year_months
